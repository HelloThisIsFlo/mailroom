...
must_haves:
  truths:
    ...
    - "required_mailboxes property conditionally includes @MailroomWarning only when warnings_enabled is True"
  artifacts:
    - path: "src/mailroom/core/config.py"
      provides: "@ToPerson label, @MailroomWarning label, warnings_enabled toggle, contact_type in mapping, required_mailboxes with conditional warning validation"
...
- Test `required_mailboxes` property includes `@MailroomWarning` when `warnings_enabled=True` (`test_startup_validates_warning_label_when_enabled`)
- Test `required_mailboxes` property does NOT include `@MailroomWarning` when `warnings_enabled=False` (`test_startup_succeeds_without_warning_label_when_disabled`)
...
- Add new entry for `self.label_to_person` in `label_to_group_mapping`:
  ...
  ```
- Add a `required_mailboxes` property that returns the full list of mailbox names needed at startup (same list pattern used in human tests lines 70-76: Inbox, screener_mailbox, label_mailroom_error, *triage_labels, *destination_mailboxes). Conditionally include `self.label_mailroom_warning` ONLY when `self.warnings_enabled is True`. NOTE: @MailroomWarning is NOT a triage label -- it's a status label like @MailroomError. It must NOT be added to `triage_labels`.

**Step 3: Add contact_type parameter to upsert_contact (carddav.py):**
- Add `contact_type: str = "company"` parameter to `upsert_contact()` method signature
- Pass `contact_type=contact_type` through to the `create_contact()` call inside upsert_contact
- This ensures the workflow can route the contact type from config mapping all the way to vCard construction

**Step 4: CardDAV create_contact (carddav.py):**
...
**Step 5: CardDAV upsert_contact NOTE behavior (carddav.py):**
...
**Step 6: Name mismatch signal in upsert_contact:**
...
<success_criteria>
- Config: 5 triage labels, all mapping entries have contact_type, @MailroomWarning + warnings_enabled fields, required_mailboxes conditionally includes @MailroomWarning
