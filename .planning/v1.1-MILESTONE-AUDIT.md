...
tech_debt:
  ...
  - phase: deployment-artifacts
    items:
      - ".env.example still references 9 deleted individual MAILROOM_LABEL_*/MAILROOM_GROUP_* env vars — should be replaced with MAILROOM_TRIAGE_CATEGORIES JSON example"
      - ".env.example shows MAILROOM_POLL_INTERVAL=300 — default changed to 60 in Phase 8 (push as primary, polling as fallback)"
      - ".env.example missing MAILROOM_DEBOUNCE_SECONDS (new Phase 8 config field, default 3)"
      - "k8s/configmap.yaml still references 9 deleted individual label/group env vars — should be replaced with MAILROOM_TRIAGE_CATEGORIES"
      - "k8s/configmap.yaml shows MAILROOM_POLL_INTERVAL 300 — should be 60"
      - "k8s/configmap.yaml missing MAILROOM_DEBOUNCE_SECONDS"
---
...
### Deployment Artifacts (cross-phase)
- **`.env.example`** still references 9 deleted individual `MAILROOM_LABEL_*`/`MAILROOM_GROUP_*` env vars — should be replaced with `MAILROOM_TRIAGE_CATEGORIES` JSON example. Shows `POLL_INTERVAL=300` (now defaults to 60). Missing `MAILROOM_DEBOUNCE_SECONDS`.
- **`k8s/configmap.yaml`** has same stale env vars — 9 deleted label/group fields, `POLL_INTERVAL: "300"`, missing `DEBOUNCE_SECONDS`. Users deploying with this ConfigMap would inject env vars that pydantic-settings silently ignores.

**Total:** 11 items across 4 areas. None are blockers but deployment artifacts would mislead users.
